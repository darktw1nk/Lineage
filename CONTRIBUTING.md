...
```
packages/core     @voxor/lineage-core — GA engine, operators, providers, persistence
...
- **ESM everywhere**; relative imports keep the `.js` extension. Consumers import from `@voxor/lineage-core`'s index only — deep imports are not part of the contract.
...
- CLI: mock the whole engine — `vi.mock('@voxor/lineage-core')` — see `packages/cli/tests/engine.test.ts`
