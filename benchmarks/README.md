...
a fraction of the price, and on easy tasks that is the right tool. What evolution buys is
**a measured answer instead of a hopeful one**: it tries many prompts, scores every one,
keeps what actually wins, and tells you on held-out data whether the gain is real. On
these five tasks that was worth +0.67 to +2.00 points over the one-shot rewrite — and it
was the only method that never made a prompt worse.
