...
- For llm_grade-heavy runs, enable `"pairwise": { "enabled": true }` — top contenders are re-ranked by head-to-head judging (both orders, position bias cancels); the champion is the playoff winner, not the noisiest 9.9. Judge calls count toward the budget.
