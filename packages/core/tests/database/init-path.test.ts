...
describe('model catalog seeding', () => {
  ...
  it('seeds a fresh database at the latest schema without retired models', async () => {
  ...
  it('migration 3 refreshes provider models but preserves openrouter rows', async () => {
    ...
    expect((db.prepare('SELECT version FROM schema_version ORDER BY version DESC LIMIT 1').get() as any).version).toBe(5);
  });
  ...
  it('migration 4 drops the legacy cost_ledger table', async () => {
    ...
    expect((db.prepare('SELECT version FROM schema_version').get() as any).version).toBe(5);
  });
